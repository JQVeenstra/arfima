...
#' 
#' grey: exact prediction red: exact prediction intervals (PIs) orange:
#' limiting PIs blue: (residual) bootstrap prediction and PIs
#' 
...
"plotpredARFIMA" <- function(xy, xlab = NULL, ylab = NULL, ylim = NULL, ...) {
    ...
    #print(length((nn + 1):(length(xy$xs) - 1)))
    #print(length(xy$exactPIU))
    ...
    if (!is.null(xy$uppernp)) {
        lines(xy$xs[nn:(length(xy$xs) - 1)], c(zlast, xy$medvalnp), col = "blue")
